...
\begin{frame}{V8 deliberately targets matrix row layout}
\begin{itemize}
\item Target: separately allocated rows in a $360\times360$ matrix, about 2880 bytes per row of doubles.
\item Split small/large regions with a reserved guard and preserve exact split sizes.
\item Use 64-byte alignment from 2048 bytes; defer page alignment until 2 MiB.
\item Bypass the small-object thread cache on the hot allocation path and retain deferred coalescing.
\end{itemize}
\takeaway{Yes, we changed the allocator for a specific implementation.}
\small The report says v8 linked-list runs were terminated before completion. They are incomplete trials, not finite slowdown estimates.
\source{Report Sections III-J/K and XVI-D. Row-locality benefits remain an implementation-based explanation.}
\end{frame}


\begin{frame}{V8 Regular versus Overloaded observations}
\centering\input{figures/v8_pairs.tex}
\par\scriptsize E1: M3 Max campaign, Arg1=360. E2: Jacob’s Ubuntu host, Arg1=275/400/550. Lines connect presumed pairs, black ticks show means. Each panel uses its own zero-based scale.
\source{Updated v8 data and figures. E1 n=3; E2 n=5. Do not compare raw time across unmatched workloads.}
\end{frame}


\begin{frame}{V8 gains do not transfer uniformly to the new group}
